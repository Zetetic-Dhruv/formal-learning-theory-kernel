...
| | Contributions |
|---|---|
| **Lean engineering and proof choices** | 1. Borel parameterization as a typeclass: any concept class parameterized by a `StandardBorelSpace` with jointly measurable evaluation satisfies `WellBehavedVCMeasTarget`. Lean 4 packaging of Pollard's permissibility (1984 App C) and Dudley's image-admissible-Suslin condition (1999 §5.3) for direct use in the σ-algebra hierarchy. Interpolation and amalgamation closures are corollaries.<br>2. Suslin projection + Choquet capacitability route from Borel witness graphs to null-measurable bad events, applied to uniform convergence bad events. Classical in empirical process theory [Pollard 1984; Dudley 1999]; mechanised here in Lean 4 with explicit σ-algebra hypotheses on the bad event itself.<br>3. Countable measurable selection via `Nat.find` on the underlying enumeration. An alternative to the K-R-N selection theorem (which is not yet in Mathlib) for the countable case; the elementary route using well-ordering of ℕ.<br>4. Compression via MWU-based approximate minimax. MWU as approximate minimax is textbook material [Freund-Schapire 1996; Cesa-Bianchi-Lugosi 2006; Arora-Hazan-Kale 2012]; substituting it for Sion's exact-minimax step in Moran-Yehudayoff (2016) yields a constructive iterative proof in place of Sion's existence argument. The specific quantitative parameter design (margin 13/24 from slack budget 1/12 + 1/24 + 1/8) is original to this formalization.<br>5. Measure-theory-free compression pipeline: `FinitePMF` + `boolTestExpectation` + `empiricalPMF` prove the compression direction with zero `MeasureTheory` dependency. Engineering choice; isolates compression from the measure-theoretic infrastructure used elsewhere.<br>6. `MeasurableBatchLearner`: Lean 4 typeclass for joint learner measurability — the standard precondition for admissibility as a measurable policy in measure-theoretic RL frameworks [Bertsekas-Shreve 1978; Puterman 1994; Devroye-Györfi-Lugosi 1996]. The kernel proves that discrete and symbolic learners (e.g., version space) satisfy this precondition, complementing the trivial case of continuous-parametric learners (e.g., neural) which clear it via continuity.<br>7. Reader-style monad on `MeasurableBatchLearner` with `pure`, `bind`, `combine`, `boost`, `interp`, `concat` operations, and `UniformMeasurableBatchFamily` for countable selection. Categorical structure is the standard Reader monad; the contribution is the formal verification that all closure operations preserve joint measurability so composite learners inherit MBL without re-derivation. |
| **Named results and witnesses** | 1. NullMeasurableSet refinement: weakens the symmetrization hypothesis used by Krapp-Wirth (2024) from `MeasurableSet` to `NullMeasurableSet`. Modern σ-algebra-explicit formulation of the Pollard-Dudley universal-measurability framework [Pollard 1984; Dudley 1999].<br>2. Borel-analytic separation: first concrete concept-class witness exhibiting the strict gap between analytic-non-Borel bad events and Borel bad events, in the role of separating Krapp-Wirth (2024)'s `MeasurableSet` hypothesis. The descriptive-set-theoretic ingredients are textbook [Kechris 1995]; the concept-class placement is the contribution. Formalized as `KrappWirthSeparationMeasTarget`, the measurable-target variant used in the fundamental theorem pipeline.<br>3. Interpolation descent: named formalization of the corollary that fixed and countable interpolations of Borel-parameterized concept classes inherit `WellBehavedVCMeasTarget`. Folklore consequence of the Pollard-Dudley framework, not previously stated as a named lemma.<br>4. Amalgamation descent: named formalization of fiber-product-amalgamation closure of Borel-parameterized classes over a common standard Borel target. Closely related to but not stated as such in DGZ (1991) or vdV-W (2000) preservation theorems. Interpolation embeds as a special case (`interpClassFixed_subset_amalgClass`).<br>5. Version space measurability: first Lean 4 formalization of joint measurability for the version-space learner [Mitchell 1977], using the countable-selection technique above. Folklore in measurable learning theory; not previously machine-checked. |
| **First formalizations** | 1. Fundamental theorem of statistical learning (5-way equivalence, all conjuncts, sorry-free).<br>2. Compression characterization (VCDim finite implies compression scheme; new proof route via approximate minimax).<br>3. Approximate minimax via multiplicative weights update (field-independent, Mathlib-contributable).<br>4. Choquet capacitability theorem (Kechris 30.13; 416 lines, Mathlib-contributable).<br>5. Analytic measurability bridge (analytic sets are NullMeasurableSet; 110 lines, Mathlib-contributable).<br>6. PAC-Bayes bound (McAllester; first frequentist-Bayesian bridge in Lean 4).<br>7. Littlestone characterization with corrected path-wise tree definition.<br>8. Gold's theorem and mind change characterization.<br>9. PAC not implies Online (threshold class on N, constructive witness).<br>10. Gold not implies PAC (finite-subset indicators, constructive witness).<br>11. Online implies PAC (unconditional).<br>12. Three-paradigm separation: PAC not implies Online, Gold not implies PAC, Online implies PAC; no proof infrastructure shared across paradigms.<br>13. Borel-analytic separation (WellBehavedVCMeasTarget holds, KrappWirthWellBehaved fails).<br>14. NFL for infinite domains (provably false for finite X).<br>15. Baxter multi-task base case.<br>16. Confidence boosting via Chebyshev concentration.<br>17. Measurability typeclass hierarchy with strict separation.<br>18. Finite-support uniform convergence (measure-theory-free). |
...
| Novel results | None | Strict-separation concept-class witness against Krapp-Wirth (2024); null-measurable refinement of the symmetrization hypothesis |
...
| Longest proof chain | ~5,800 lines (compression: MWU + Binary Matrix + VC-approx + symmetrization) |
...
### Named results (first machine-checked)

These are classical or folklore results given a named Lean 4 form for the first time. The mathematical content is not new; the contribution is the formalization and the named theorem available for downstream use.

| Theorem | File | Result | Prior art |
|---------|------|--------|-----------|
| `AnalyticSet.cap_eq_iSup_isCompact` | PureMath/ChoquetCapacity.lean | Choquet capacitability for analytic sets | Choquet 1955; Kechris 1995, Thm 30.13 |
| `analyticSet_nullMeasurableSet` | PureMath/AnalyticMeasurability.lean | Analytic sets are NullMeasurableSet | Pollard 1984 App C; Dudley 1999 §5.3; folklore in EP theory |
| `interpolation_descent` | Complexity/Interpolation.lean | Composition of Borel-parameterized classes weakens measurability | Folklore; cf. DGZ 1991, vdV-W 2000 |
| `versionSpaceLearner_measurableBatchLearner` | Learner/VersionSpace.lean | Version space learners satisfy MeasurableBatchLearner | Folklore in measurable learning theory |
| `chebyshev_majority_bound` | PureMath/Concentration.lean | Majority vote concentration bound | Chebyshev's inequality; standard textbook |
