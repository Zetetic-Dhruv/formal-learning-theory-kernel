...
The Borel-analytic separation (Section III) proves the gap is strict: there exist concept classes where `WellBehavedVCMeasTarget` holds but `KrappWirthWellBehaved` (Borel measurability) fails. `NullMeasurableSet` is not just a convenience. It is the correct level for the measurable-target condition used in the fundamental theorem.
