...
on $m + d$.  It is given in full in \Cref{ch:dimensions}
(\Cref{lem:sauer-shelah-proof}).  For the present chapter, we take it as given
...
Littlestone dimension implies finite VC dimension (\Cref{ch:dimensions}), but
...
\item $\ref{F:online} \Rightarrow \ref{F:finvc}$: If $\Ldim(\mathcal{H}) < \infty$,
  ...
  set gives a complete binary mistake tree of the same depth (\Cref{ch:dimensions}).
\end{itemize}
...
deep one.  It was an open conjecture for 30 years before being resolved by
Moran and Yehudayoff~\cite{MoranYehudayoff2016}.

\begin{historical}
\textbf{The compression conjecture.}
Littlestone and Warmuth conjectured in 1986 that every class of VC dimension
$d$ has a compression scheme of size $O(d)$.  The conjecture was resolved
positively by Moran and Yehudayoff (2016), who proved that every class of
VC dimension $d$ has a compression scheme of size at most $2^{O(d)}$.
The $O(d)$ bound remains open; the exponential gap between $d$ and $2^{O(d)}$
...
\Cref{thm:nfl} in \Cref{ch:objects} gave the statement and a brief argument.
...
\Cref{ch:computational}.  The key examples:
\begin{itemize}[nosep]
...
\item \textbf{Information $\neq$ computation.}  The Fundamental Theorem
  ...
  computational landscape is the subject of \Cref{ch:computational}.
\end{enumerate}
