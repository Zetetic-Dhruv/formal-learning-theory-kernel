...
|---------|----------------------|----------------------|-----------------|
| Fundamental theorem | VCDim(C) < infinity | PACLearnable(C) | Combinatorics ↔ Measure theory |
| Littlestone char. | LittlestoneDim(C) < infinity | OnlineLearnable(C) | Combinatorics ↔ Game theory |
| Mind change char. | MindChangeOrdinal < omega | EXLearnable(C) | Ordinal arithmetic ↔ Computability |
| PAC-Bayes | KL(Q \|\| P) penalty | Gibbs error bound | Information theory ↔ Measure theory |
...
**In each biconditional above, the constructive direction (structural → algorithmic: "build a learner from a bound") requires more proof infrastructure than the non-constructive direction (algorithmic → structural: "if a learner exists, derive the bound").**
...
| Mind change char. | Absorbed into definition (see below), ~0 proof lines | Finite change set + convergence, ~30 lines | see below |

The pattern: building the algorithm from the structural bound is harder than proving the structural bound from the algorithm's existence. This predicts that for any new characterization theorem in this kernel, the constructive direction will dominate the proof burden.
...
The constructive direction's infrastructure is inside the definition, not in the proof. The total work (definition + proof) preserves the asymmetry: the constructive content (encoding correctness in the return type) is the harder design problem, even though it produces zero lines of proof.

This is the first instance of a pattern examined in Section IV: the choice of definition determines where proof work lives.
...
McAllester's PAC-Bayes bound connects three mathematical domains:
...
| PAC learning | Generalization guarantee | The bound on expected error of the Gibbs classifier |
